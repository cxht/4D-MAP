...
//func New(conn quic.Stream, modOptions ...ModOption) Connection {
	c := new(connection)
...
// func(c *connection) ReturnMonitor()(flag bool,limit int){
// 	return c.Conn.ReturnMonitor()
// }
...
func (c *connection) close(err error) {
	c.closeOnce.Do(func() {
		...
		if(err.Error()  == "find discard err" ){
			nazalog.Debugf("[%s] dont close err=%+v", c.uniqueKey, err)
		}else{
