...
type receivedPacketHandler struct {
	...
	//cx add for throughput estimate
...
//cx add 
func  (h *receivedPacketHandler) GetStatisticstp() (float64,float64) {
	now := time.Now()
...
func (h *receivedPacketHandler) ReceivedPacket(packetNumber protocol.PacketNumber, shouldInstigateAck bool, size int) error {
	if packetNumber == 0 {
	...
	h.packets++
	h.totalrecvsize += float64(size)
	h.thistimesize += float64(size)
