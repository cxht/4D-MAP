...
func Message2Chunks(message []byte, header *base.RTMPHeader, mark bool) []byte {
	return defaultChunkDivider.Message2Chunks(message, header, mark)
}

// TODO chef: 新的 message 的第一个 chunk 始终使用 fmt0 格式，没有参考前一个 message
func (d *ChunkDivider) Message2Chunks(message []byte, header *base.RTMPHeader, mark bool) []byte {
	return message2Chunks(message, header, nil, d.localChunkSize, mark)
}
...
func calcHeader(header *base.RTMPHeader, prevHeader *base.RTMPHeader, out []byte) int {
	...
	if header.CSID >= 2 && header.CSID <= 63 {
		...
		//log.Infof("headerfirstbyte:%v",out[index])
		index++
...
func message2Chunks(message []byte, header *base.RTMPHeader, prevHeader *base.RTMPHeader, chunkSize int, mark bool) []byte {
	...
	//fmt.Println("chunksize",chunkSize, numOfChunk)
	...
	for i := 0; i < numOfChunk; i++ {
		...
		//nazalog.Infof("headerlen:%v,index:%v",headLen,index)
		index += headLen
