...
type streamFrameSorter struct {
	...
	dupframesize protocol.ByteCount
}
...
func (s *streamFrameSorter) Push(frame *wire.StreamFrame) error {
	...
	if end > endGap.Value.End {
		...
		wasCut = true
		utils.Infof("was cut :%v", frame.Data)
	}
	...
	if s.gaps.Len() > protocol.MaxStreamFrameSorterGaps {
		utils.Infof("[sfs]gaps len:%v",s.gaps.Len())
...
func (s *streamFrameSorter) Pop() *wire.StreamFrame {
	frame := s.Head()
	rcvtime := frame.Rcvpkttime			//cx :for ooo delay
	now := time.Now()
	ooo := float64(now.Sub(rcvtime).Milliseconds())
	utils.Debugf("ooo:%fe",ooo)
	//utils.Debugf("frame len:%v, off:%v",frame.DataLen(),frame.Offset)
	if frame != nil {
...
func (s *streamFrameSorter) Head() *wire.StreamFrame {
	...
	//utils.Infof("red pos:%v",s.readPosition)
	if ok {
		//utils.Infof("normal frame:%v,datalen:%v,off:%v",frame,)
		return frame
	}
	// utils.Debugf("not ok %d",cnok)
	// cnok+=1
	return nil

}
func (s *streamFrameSorter) FullZeroFrame(offset protocol.ByteCount) *wire.StreamFrame {
	...
	//utils.Debugf("[str]fullzerorframe: %v",content)
	return &wire.StreamFrame{StreamID:protocol.StreamID(3),FinBit: false, Offset: offset, Data:content,Priority:false}
}