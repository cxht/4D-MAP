...
func LogFrame(frame Frame, sent bool) {
	...
	switch f := frame.(type) {
	case *StreamFrame:
		//utils.Debugf("\t%s &wire.StreamFrame{StreamID: %d, FinBit: %t, Offset: 0x%x, Data length: %d, Offset + Data length: 0x%x, \n Data: %v}", dir, f.StreamID, f.FinBit, f.Offset, f.DataLen(), f.Offset+f.DataLen(), f.Data)
		utils.Debugf("\t%s &wire.StreamFrame{StreamID: %d, FinBit: %t, Offset: 0x%x, Data length: %d, Offset + Data length: 0x%x }", dir, f.StreamID, f.FinBit, f.Offset, f.DataLen(), f.Offset+f.DataLen())
